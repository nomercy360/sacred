...
const NumberFieldInput = <T extends ValidComponent = 'input'>(
	...
	return (
		<NumberFieldPrimitive.Input
			ref={setInput}
			class={cn(
				'flex h-16 text-xl w-full rounded-md bg-transparent px-3 py-2 ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 data-[invalid]:border-error-foreground data-[invalid]:text-error-foreground',
				local.class,
			)}
			{...others}
		/>
...
const NumberFieldIncrementTrigger = <T extends ValidComponent = 'button'>(
	...
	return (
		<NumberFieldPrimitive.IncrementTrigger
			class={cn(
				'absolute right-1 top-1.5 inline-flex size-4 items-center justify-center',
				local.class,
			)}
			{...others}
		>
...
const NumberFieldDecrementTrigger = <T extends ValidComponent = 'button'>(
	...
	return (
		<NumberFieldPrimitive.DecrementTrigger
			class={cn(
				'absolute bottom-1.5 right-1 inline-flex size-4 items-center justify-center',
				local.class,
			)}
			{...others}
		>
