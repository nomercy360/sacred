...
export const autocompleteSearch = async (
    ...
    const { data } = await apiRequest(
        `/feed/autocomplete?search=${encodeURIComponent(query)}`,
        {
            method: 'GET',
        },
    )
...
export const fetchUpdateWish = async (
    id: string,
    body: UpdateWishRequest & { photos?: File[] },
) => {
