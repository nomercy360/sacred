...
export default function UserBoardPage() {
  ...
  const groupedWishes = createMemo(() => {
    ...
    a.length === b.length && a.every((group, i) => 
      group.length === b[i].length && group.every((wish, j) => wish.id === b[i][j].id)
    )
  })
    ...
    <div class="relative flex flex-col items-center w-full h-screen overflow-y-auto pb-20">
      ...
      <div class="text-center w-full pt-20">
        <Switch>
          ...
          <Match when={!store.wishes.length && wishesResource.state === 'ready'}>
            <div class="text-gray-400 text-center pt-10 font-medium text-lg">
              No wishes yet...
            </div>
          </Match>
        </Switch>
